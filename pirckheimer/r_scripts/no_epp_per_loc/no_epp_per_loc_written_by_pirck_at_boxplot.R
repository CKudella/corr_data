...
upper_whisker <- max(data$Number.of.letters.sent.from.this.location.from.Pirckheimer[data$Number.of.letters.sent.from.this.location.from.Pirckheimer < (quartiles[3] + 1.58 * IQR)])
...
plot <- ggplot(data, aes(x = " ", y = Number.of.letters.sent.from.this.location.from.Pirckheimer)) +
  ...
  geom_text_repel(label = ifelse(data$Number.of.letters.sent.from.this.location.from.Pirckheimer > upper_whisker, as.character(data$Location.Name), "")) +
  theme_bw() +
