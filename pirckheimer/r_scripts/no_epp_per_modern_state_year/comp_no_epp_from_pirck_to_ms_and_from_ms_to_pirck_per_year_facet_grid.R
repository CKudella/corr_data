...
plot <- ggplot(data = data_long, aes(x = Year, y = value, colour = variable)) +
  ...
  scale_colour_discrete(labels = c("Number of letters sent by Pirckheimer to this modern state per year", "Number of letters sent to Pirckheimer from this modern state per year"))
plot
