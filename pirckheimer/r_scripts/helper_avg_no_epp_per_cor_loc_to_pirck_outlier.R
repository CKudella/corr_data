...
# read data for no epp per loc written to pirck
data_epp<-read.csv("no_epp_per_loc/no_epp_per_loc_sent_to_pirck_with_geocoordinates.csv", fileEncoding="UTF-8", na.strings=c("NULL"))

# read data for avg no epp per corr loc written to pirck
data_avg <-read.csv("no_epp_per_loc/avg_no_epp_per_corr_loc_written_to_pirck.csv", fileEncoding="UTF-8", na.strings=c("NULL"))

# read data for no corr per loc writing to pirck
data_corr<-read.csv("no_corr_per_loc/no_corr_per_loc_writing_to_pirck_with_geocoordinates.csv", fileEncoding="UTF-8", na.strings=c("NULL"))

# subset data_avg outliers
data_avg_outlier <- subset(data_avg, data_avg$Average.Number.of.Letters >= 3.5)

# subset data_epp outliers
data_epp_outlier <- subset(data_epp, data_epp$Number.of.Letters.sent.from.this.location.TO.Pirckheimer >= 8.5)

# subset data_corr outliers
data_corr_outlier <- subset(data_corr, data_corr$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer >= 3.5)
