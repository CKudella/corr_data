...
upper_whisker <- max(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer[data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer < (quartiles[3] + 1.58 * IQR)])
...
plot2 <- ggplot(data, aes(x = " ", y = Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer)) +
  ...
  geom_text_repel(label = ifelse(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer > upper_whisker, as.character(data$Location.Name.Modern), "")) +
  theme_bw() +
