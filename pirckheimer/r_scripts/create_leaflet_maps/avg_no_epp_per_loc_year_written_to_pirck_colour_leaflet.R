...
# Read data
data <- read.csv("no_epp_per_loc/avg_no_epp_per_loc_year_written_to_pirck.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Determine breaks
median_val <- median(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year, na.rm = TRUE)
q1 <- quantile(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year, 0.25, na.rm = TRUE)
q3 <- quantile(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year)
...
breaks <- c(
  min(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year, na.rm = TRUE),
  q3, outliers, max(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year, na.rm = TRUE)
)
...
data$class <- findInterval(data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Average number of letters sent to <br/> Pirckheimer from this location per year: ", data$Average.number.of.letters.written.from.this.location.to.Pirckheimer.per.year),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
# Ensure all breaks have 2 decimal places
format_numbers <- function(x) sprintf("%.2f", x) # Format to 2 decimal places
adjust_value <- 0.01  # Keep adjustment consistent
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Average number of letters sent to Pirckheimer from this location per year</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "avg_no_epp_per_loc_year_written_to_pirck_colour.html", selfcontained = TRUE)
