...
# Determine breaks
median_val <- median(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer, na.rm = TRUE)
q1 <- quantile(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer)
...
breaks <- c(
  min(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer, na.rm = TRUE),
  q3, outliers, max(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer, na.rm = TRUE)
)
...
data$class <- findInterval(data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name.Modern, "</b><br>Number of correspondents at this <br/> location to whom Pirckheimer wrote letters: ", data$Number.of.correspondents.who.received.at.this.Location.letters.from.Pirckheimer),
    group = "Locations",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of correspondents at this location to whom Pirckheimer wrote letters</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_corr_per_loc_receiving_from_pirck_with_geocoordinates_outliers_colour.html", selfcontained = TRUE)
