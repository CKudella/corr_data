...
num_breaks <- 4
breaks <- classIntervals(data$Number.of.Letters.sent.to.this.location.by.Pirckheimer, n = num_breaks, style = "jenks")$brks
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Number of letters sent by <br/> Pirckheimer to this location: ", data$Number.of.Letters.sent.to.this.location.by.Pirckheimer),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
saveWidget(m, file = "no_epp_per_loc_sent_by_pirck_to_with_geocoordinates_colour.html", selfcontained = TRUE)
