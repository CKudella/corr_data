...
breaks <- c(
  min(data$Average.Number.of.Letters, na.rm = TRUE),
  q3, outliers, max(data$Average.Number.of.Letters, na.rm = TRUE)
)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Average number of letters sent to Pirckheimer <br> from this location per correspondent ", data$Average.Number.of.Letters),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Average number of letters sent to Pirckheimer from this location per correspondent</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "avg_no_epp_per_corr_loc_written_to_pirck_colour.html", selfcontained = TRUE)
