...
# Read data
data <- read.csv("no_epp_per_loc/avg_no_epp_per_loc_year_written_by_pirck_to.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Determine breaks
median_val <- median(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year, na.rm = TRUE)
q1 <- quantile(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year, 0.25, na.rm = TRUE)
q3 <- quantile(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year)
...
breaks <- c(
  min(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year, na.rm = TRUE),
  q3, outliers, max(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year, na.rm = TRUE)
)
...
data$class <- findInterval(data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Average number of letters sent by <br/> Pirckheimer to this location per year: ", data$Average.number.of.letters.written.by.Pirckheimer.to.this.location.per.year),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Average number of letters sent by Pirckheimer to this location per year</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "avg_no_epp_per_loc_year_written_by_pirck_to_colour.html", selfcontained = TRUE)
