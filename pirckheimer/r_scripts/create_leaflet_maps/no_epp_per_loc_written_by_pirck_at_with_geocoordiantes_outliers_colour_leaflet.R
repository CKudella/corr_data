...
# Read data
data <- read.csv("no_epp_per_loc/no_epp_per_loc_written_by_pirck_at_with_geocoordinates.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Determine breaks
median_val <- median(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer, na.rm = TRUE)
q1 <- quantile(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer)
...
breaks <- c(
  min(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer, na.rm = TRUE),
  median_val, q3, max(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer, na.rm = TRUE)
)

# Ensure breaks are treated as a numeric vector
breaks_numeric <- as.numeric(breaks)

# Round all breaks to the nearest whole number
breaks_numeric <- round(breaks_numeric)
...
data$class <- findInterval(data$Number.of.Letters.sent.from.this.location.by.Pirckheimer, breaks_numeric, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Number of letters written by <br> Pirckheimer at this location: ", data$Number.of.Letters.sent.from.this.location.by.Pirckheimer),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters written by Pirckheimer at this location</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_written_by_pirck_at_with_geocoordiantes_outliers_colour.html", selfcontained = TRUE)
