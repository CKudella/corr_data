...
# Read data
nodes <- read.csv("create_geolayout_files_for_gephi/nodes_edges_for_epp_sent_by_pirck/create_nodes_locations_for_epp_sent_by_pirck.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))
edges <- read.csv("create_geolayout_files_for_gephi/nodes_edges_for_epp_sent_by_pirck/create_edges_epp_sent_by_pirck.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))
data_for_breaks <- read.csv("no_epp_per_loc/no_epp_per_loc_sent_by_pirck_to_with_geocoordinates.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Select only the relevant columns from letter_counts
letter_counts <- data_for_breaks %>%
  select(Location.Name, Number.of.Letters.sent.to.this.location.by.Pirckheimer)

# Merge letter_counts with nodes, adding only the 'degree' column
nodes <- nodes %>%
  left_join(letter_counts, by = c("Label" = "Location.Name")) %>%
  rename(degree = Number.of.Letters.sent.to.this.location.by.Pirckheimer) # Rename the column to 'degree'
...
# Determine breaks
median_val <- median(data_for_breaks$Number.of.Letters.sent.to.this.location.by.Pirckheimer, na.rm = TRUE)
q1 <- quantile(data_for_breaks$Number.of.Letters.sent.to.this.location.by.Pirckheimer, 0.25, na.rm = TRUE)
q3 <- quantile(data_for_breaks$Number.of.Letters.sent.to.this.location.by.Pirckheimer, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data_for_breaks$Number.of.Letters.sent.to.this.location.by.Pirckheimer)
...
breaks <- c(
  min(data_for_breaks$Number.of.Letters.sent.to.this.location.by.Pirckheimer, na.rm = TRUE),
...
m <- m %>%
  addPulseMarkers(
    ...
    popup = ~ paste0("<b>", Label, "</b><br>Number of letters sent by Pirckheimer within this city: ", weight), # Use weight
...
m <- m %>%
  addCircleMarkers(
    ...
    lng = ~locations_lng,
    lat = ~locations_lat,
    popup = paste("<b>", nodes$Label, "</b><br>Number of letters sent by Pirckheimer to this location : ", nodes$degree),
    group = "Nodes",
    label = ~Label,
    radius = 2,
    fillColor = "#C3161F",
...
legend_html <- paste0(
  ...
  paste0(
    sapply(1:(length(breaks_numeric) - 1), function(i) {
      ...
      raw_upper_bound <- breaks_numeric[i + 1] # Store original value before modification
      upper_bound <- if (i == length(breaks_numeric) - 1) raw_upper_bound else raw_upper_bound - adjust_value

      # Use adjusted upper_bound in the check
      if (lower_bound == upper_bound) {
        label_text <- if (lower_bound == 1) {
          paste0(format_numbers(lower_bound), " letter")
        } else {
          paste0(format_numbers(lower_bound), " letters")
        }
      } else {
...
saveWidget(m, file = "nodes_edges_for_epp_sent_by_pirck_colour.html", selfcontained = TRUE)
m
