...
num_breaks <- 6
breaks <- classIntervals(data$Number.of.Letters.sent.from.this.location.TO.Pirckheimer, n = num_breaks, style = "jenks")$brks
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Number of letters sent to <br/> Pirckheimer from this location: ", data$Number.of.Letters.sent.from.this.location.TO.Pirckheimer),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
saveWidget(m, file = "no_epp_per_loc_sent_to_pirck_with_geocoordinates_colour.html", selfcontained = TRUE)
