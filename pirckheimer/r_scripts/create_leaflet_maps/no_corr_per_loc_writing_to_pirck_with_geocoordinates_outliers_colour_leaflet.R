...
# Determine breaks
median_val <- median(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer, na.rm = TRUE)
q1 <- quantile(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer)
...
breaks <- c(
  min(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer, na.rm = TRUE),
  q3, outliers, max(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer, na.rm = TRUE)
)
...
data$class <- findInterval(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name.Modern, "</b><br>Number of correspondents who wrote <br/> from this location to Pirckheimer: ", data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Pirckheimer),
    group = "Locations",
    ...
    sapply(1:(length(breaks_numeric) - 1), function(i) {
      ...
      if (lower_bound == upper_bound) {
        label_text <- if (lower_bound == 1) {
          paste0(format_numbers(lower_bound), " correspondent")
        } else {
          paste0(format_numbers(lower_bound), " correspondents")
        }
      } else {
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of correspondents who wrote from this location to Pirckheimer</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_corr_per_loc_writing_to_pirck_with_geocoordinates_outliers_colour.html", selfcontained = TRUE)
