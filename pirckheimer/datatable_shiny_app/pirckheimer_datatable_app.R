...
ui <- fluidPage(
  ...
  titlePanel("Pirckheimer Query Interface - Proof of Concept"),
  fluidRow(
...
server <- function(input, output) {
  # Load data
  letters <- read.csv("../dataset/csv/wpirck_letters.csv",
                      fileEncoding = "UTF-8",
                      na.strings = c("")
  )
  
  locations <- read.csv("../dataset/csv/wpirck_locations.csv",
                        fileEncoding = "UTF-8",
                        na.strings = c("")
  )
