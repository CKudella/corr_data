SELECT X.LocationName, X.Latitude, X.Longitude, X.NoLettersWrittenTOErasmus, Y.NoLettersWrittenBYErasmusTO FROM
(
...
(
SELECT YB.locations_name_modern AS LocationName, YB.locations_lat AS 'Latitude', YB.locations_lng as 'Longitude', COUNT(YA.target_loc_id) AS NoLettersWrittenBYErasmusTO FROM letters AS YA JOIN locations AS YB ON YB.locations_id = YA.target_loc_id WHERE YA.letters_id NOT LIKE '%ck2' AND YA.recipient_id != 'erasmus_desiderius_viaf_95982394' GROUP BY YA.target_loc_id ORDER BY COUNT(YA.target_loc_id) DESC
