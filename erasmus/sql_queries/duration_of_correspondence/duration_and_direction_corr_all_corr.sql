SELECT C.correspondents_id,
       ...
       X.FLFE AS 'First letter FROM Erasmus TO this correspondent',
       X.LLFE AS 'Last letter FROM Erasmus TO this correspondent',
       Y.FLTE AS 'First letter FROM this correspondent TO Erasmus',
       Y.LLTE AS 'Last letter FROM this correspondent TO Erasmus',
       CASE
           WHEN X.FLFE IS NOT NULL THEN 1
           ELSE 0
       END AS 'Has letters FROM Erasmus',
       CASE
           WHEN Y.FLTE IS NOT NULL THEN 1
           ELSE 0
       END AS 'Has letters TO Erasmus'
FROM
