...
upper_whisker <- max(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus[data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus < (quartiles[3] + 1.58 * IQR)])
...
plot2 <- ggplot(data, aes(x = " ", y = Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus)) +
  ...
  geom_text_repel(label = ifelse(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus > upper_whisker, as.character(data$Location.Name.Modern), "")) +
  theme_bw() +
