...
data_path <- "../query_results/duration_of_correspondence/duration_corr_only_epp_from_erasmus.csv"
output_path <- "../r_plots/duration_of_correspondence/duration_corr_only_epp_from_erasmus_barchart_quantile_bins"
...
dataset_label <- str_glue("correspondents with only letters FROM {subject_name} surviving")
...
bin_edges <- quantile(duration_multi, probs = seq(0, 1, length.out = n_bins + 1), na.rm = TRUE)
...
cat("\nbin edges (quantile-based, years):\n")
...
plot_categories <- category_summary |>
  ...
  labs(
    ...
    subtitle = str_glue(
      ...
      "quartile-based bins calculated on multi-occasion contacts only"
    ),
    x = "Duration category",
