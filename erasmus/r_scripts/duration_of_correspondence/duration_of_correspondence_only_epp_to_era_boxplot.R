...
duration_of_correspondence_only_epp_to_erasmus$name_in_edition <- gsub("\\b(\\W+COE+.*)", "", duration_of_correspondence_only_epp_to_erasmus$name_in_edition)
duration_of_correspondence_only_epp_to_erasmus$name_in_edition <- gsub("^(\\W+E)", "E", duration_of_correspondence_only_epp_to_erasmus$name_in_edition)
duration_of_correspondence_only_epp_to_erasmus$name_in_edition <- gsub("^\\[", "", duration_of_correspondence_only_epp_to_erasmus$name_in_edition)
...
# calculate quartiles
quartiles <- as.numeric(quantile(duration_of_correspondence_only_epp_to_erasmus$duration_in_years, probs = c(0.25, 0.5, 0.75)))

# calculate IQR
IQR <- diff(quartiles[c(1, 3)])

# calculate outlier treshold
upper_dots <- min(duration_of_correspondence_only_epp_to_erasmus$duration_in_years[duration_of_correspondence_only_epp_to_erasmus$duration_in_years > (quartiles[3] + 1.5*IQR)])

# create box plot
plot <- ggplot(duration_of_correspondence_only_epp_to_erasmus, aes(x = " ", y = duration_in_years)) +
  geom_boxplot(notch = FALSE) +
  geom_text_repel(label = ifelse(duration_of_correspondence_only_epp_to_erasmus$duration_in_years >= upper_dots, as.character(duration_of_correspondence_only_epp_to_erasmus$name_in_edition), ""), box.padding = 1.2, max.overlaps = Inf, size = 2.5) +
