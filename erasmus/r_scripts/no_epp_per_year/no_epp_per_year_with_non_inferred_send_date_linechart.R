...
# Linechart
plot <- ggplot(data=daten, aes(x= Year,y=No.of.Letters.with.non.inferred.Date.of.Dispatch)) + geom_line(stat = "identity", size=0.9) + geom_point(shape=1, size=2.5) + labs(x="Year",y="Number of letters") + scale_x_continuous(breaks = c(1484:1536)) + scale_y_continuous(breaks = seq(0,160,10))
