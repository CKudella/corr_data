...
daten<-read.csv("data/comp_date_inferred_noninferred_per_year_era_as_recipient.csv")
head(daten)
...
plot + theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.35)) + theme(legend.position="bottom") + scale_fill_grey(labels = c("Letters written to Erasmus with inferred dating", "Letters written to Erasmus with non-inferred dating"))