...
data_path <- "../query_results/no_epp_per_year/no_epp_per_year_with_inferred_send_date.csv"
output_path <- "../r_plots/no_epp_per_year/no_epp_per_year_with_inferred_send_date_barchart"

measure_col <- "Number of letters with inferred send date sent this year"
year_col    <- "Year"
...
cat(str_glue("total letters (inferred, all correspondence): {n_total}"), "\n")
...
plot_barchart <- data |>
  ...
  labs(
    title = "Letters with an inferred send date, per year (all correspondence)",
    x = "Year",
    y = "Number of letters"
  ) +
