...
data_path <- "../query_results/no_epp_per_year/no_epp_per_year_with_inferred_send_date_sent_to_era.csv"
output_path <- "../r_plots/no_epp_per_year/no_epp_per_year_with_inferred_send_date_sent_to_era_barchart"

measure_col <- str_glue("Number of letters with inferred send date sent to {subject_name} this year")
year_col    <- "Year"
...
cat(str_glue("total letters (inferred, sent to {subject_name}): {n_total}"), "\n")
...
plot_barchart <- data |>
  ...
  labs(
    title = str_glue("Letters sent TO {subject_name} with an inferred send date, per year"),
    x = "Year",
    y = "Number of letters"
  ) +
