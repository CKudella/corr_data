...
data_path <- "../query_results/no_epp_per_year/no_epp_per_year_sent_by_era_send_date_year1.csv"
output_path <- "../r_plots/no_epp_per_year/no_epp_per_year_sent_by_era_barchart"

measure_col <- str_glue("Total number of letters sent by {subject_name} this year")
year_col    <- "Year"
...
cat(str_glue("total letters sent by {subject_name}: {n_total}"), "\n")
...
plot_barchart <- data |>
  ...
  labs(
    title = str_glue("Number of letters sent BY {subject_name}, per year"),
    x = "Year",
    y = "Number of letters"
  ) +
