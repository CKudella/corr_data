...
# Determine breaks
median_val <- median(data$Number.of.letters.sent.from.this.location.to.Erasmus, na.rm = TRUE)
q1 <- quantile(data$Number.of.letters.sent.from.this.location.to.Erasmus, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.letters.sent.from.this.location.to.Erasmus, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.letters.sent.from.this.location.to.Erasmus)
...
breaks <- c(min(data$Number.of.letters.sent.from.this.location.to.Erasmus, na.rm = TRUE), 
            median_val, q3, outliers, max(data$Number.of.letters.sent.from.this.location.to.Erasmus, na.rm = TRUE))
...
data$class <- findInterval(data$Number.of.letters.sent.from.this.location.to.Erasmus, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Number of letters sent from <br/> this location to Erasmus: ", data$Number.of.letters.sent.from.this.location.to.Erasmus),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters sent to Erasmus from this location</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_sent_to_era_with_geocoordinates_outliers_colour.html", selfcontained = TRUE)
