...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Average number of letters sent <br/> by Erasmus per correspondent: ", data$Average.Number.of.letters.per.correspondent),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Average number of letters sent by Erasmus to this location per correspondent</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "avg_no_epp_per_corr_loc_written_by_era_to_colour.html", selfcontained = TRUE)
