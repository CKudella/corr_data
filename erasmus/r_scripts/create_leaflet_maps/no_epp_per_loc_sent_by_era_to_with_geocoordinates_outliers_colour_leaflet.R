...
breaks <- c(min(data$Number.of.letters.sent.to.this.location.by.Erasmus, na.rm = TRUE), 
            median_val, q3, outliers, max(data$Number.of.letters.sent.to.this.location.by.Erasmus, na.rm = TRUE))
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Number of letters sent by <br/> Erasmus this location: ", data$Number.of.letters.sent.to.this.location.by.Erasmus),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
