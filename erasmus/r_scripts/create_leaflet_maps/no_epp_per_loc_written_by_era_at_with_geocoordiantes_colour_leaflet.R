...
num_breaks <- 4
breaks <- classIntervals(data$Number.of.letters.sent.from.this.location.by.Erasmus, n = num_breaks, style = "jenks")$brks
...
data$class <- findInterval(data$Number.of.letters.sent.from.this.location.by.Erasmus, breaks, rightmost.closed = TRUE)
...
saveWidget(m, file = "no_epp_per_loc_written_by_era_at_with_geocoordiantes_colour.html", selfcontained = TRUE)
