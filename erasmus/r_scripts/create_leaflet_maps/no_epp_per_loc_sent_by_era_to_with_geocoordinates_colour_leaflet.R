...
# Determine breaks
median_val <- median(data$Number.of.letters.sent.to.this.location.by.Erasmus, na.rm = TRUE)
q1 <- quantile(data$Number.of.letters.sent.to.this.location.by.Erasmus, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.letters.sent.to.this.location.by.Erasmus, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.letters.sent.to.this.location.by.Erasmus)
upper_whisker <- box_stats$stats[5]
outliers <- min(box_stats$out)

# Calculate natural breaks
num_breaks <- 5
breaks <- classIntervals(data$Number.of.letters.sent.to.this.location.by.Erasmus, n = num_breaks, style = "jenks")$brks
...
data$class <- findInterval(data$Number.of.letters.sent.to.this.location.by.Erasmus, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Number of letters sent by <br/> Erasmus to this location: ", data$Number.of.letters.sent.to.this.location.by.Erasmus),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters sent by Erasmus to this location</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_sent_by_era_to_with_geocoordinates_colour.html", selfcontained = TRUE)
