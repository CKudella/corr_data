...
data_path   <- "../query_results/no_corr_per_loc/no_corr_per_loc_writing_to_era_with_geocoordinates.csv"
output_path <- "../leaflet_maps/no_corr_per_loc_writing_to_era_with_geocoordinates_outliers_colour.html"

measure_col  <- str_glue("Number of correspondents who wrote from this location letters to {subject_name}")
location_col <- "Location Name Modern"

breaks_method <- "boxplot"
n_classes     <- 4
unit_singular <- "correspondent"
unit_plural   <- "correspondents"

description <- str_glue("Number of correspondents who wrote from this location to {subject_name}")
