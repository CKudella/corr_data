...
# Determine breaks
median_val <- median(data$Number.of.correspondents.who.received.at.this.location.letters.from.Erasmus, na.rm = TRUE)
q1 <- quantile(data$Number.of.correspondents.who.received.at.this.location.letters.from.Erasmus, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.correspondents.who.received.at.this.location.letters.from.Erasmus, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.correspondents.who.received.at.this.location.letters.from.Erasmus)
upper_whisker <- box_stats$stats[5]
outliers <- min(box_stats$out)

# Calculate natural breaks
num_breaks <- 5
breaks <- classIntervals(data$Number.of.correspondents.who.received.at.this.location.letters.from.Erasmus, n = num_breaks, style = "jenks")$brks
...
data$class <- findInterval(data$Number.of.correspondents.who.received.at.this.location.letters.from.Erasmus, breaks, rightmost.closed = TRUE)
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of correspondents at this location to whom Erasmus wrote letters</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_corr_per_loc_receiving_from_era_with_geocoordinates_colour.html", selfcontained = TRUE)
