...
# Determine breaks
median_val <- median(data$Number.of.letters.sent.from.this.location.by.Erasmus, na.rm = TRUE)
q1 <- quantile(data$Number.of.letters.sent.from.this.location.by.Erasmus, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.letters.sent.from.this.location.by.Erasmus, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.letters.sent.from.this.location.by.Erasmus)
...
breaks <- c(min(data$Number.of.letters.sent.from.this.location.by.Erasmus, na.rm = TRUE), 
            median_val, q3, outliers, max(data$Number.of.letters.sent.from.this.location.by.Erasmus, na.rm = TRUE))
...
data$class <- findInterval(data$Number.of.letters.sent.from.this.location.by.Erasmus, breaks_numeric, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Number of letters written by <br> Erasmus at this location: ", data$Number.of.letters.sent.from.this.location.by.Erasmus),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters written by Erasmus at this location</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_written_by_era_at_with_geocoordiantes_outliers_colour.html", selfcontained = TRUE)
