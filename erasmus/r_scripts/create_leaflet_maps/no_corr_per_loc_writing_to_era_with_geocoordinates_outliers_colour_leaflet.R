...
# Determine breaks
median_val <- median(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus, na.rm = TRUE)
q1 <- quantile(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus)
...
breaks <- c(min(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus, na.rm = TRUE), 
            q3, outliers, max(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus, na.rm = TRUE))
...
data$class <- findInterval(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name.Modern, "</b><br>Number of correspondents who wrote <br/> from this location to Erasmus: ", data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Erasmus),
    group = "Locations",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of correspondents who wrote from this location to Erasmus</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_corr_per_loc_writing_to_era_with_geocoordinates_outliers_colour.html", selfcontained = TRUE)
