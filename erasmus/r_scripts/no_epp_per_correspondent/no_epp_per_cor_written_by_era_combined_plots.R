...
data_path <- "../query_results/no_epp_per_correspondent/no_epp_per_cor_written_by_era.csv"
output_path <- "../r_plots/no_epp_per_correspondent/no_epp_per_cor_written_by_era_combined_plots"

id_col      <- "recipient_id"
name_col    <- "name_in_edition"
measure_col <- str_glue("Number of letters sent by {subject_name} to this correspondent")
...
plot_histogram <- data |>
  ...
  labs(
    title = str_glue("Distribution of letters sent by {subject_name}, per correspondent"),
...
plot_boxplot <- data |>
  ...
  labs(
    title = "Correspondents receiving an outlying number of letters",
    x = NULL,
    y = "Number of letters"
  ) +
  theme_bw(base_size = 12) +
