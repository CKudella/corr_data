...
upper_whisker <- max(data$Number.of.letters.sent.from.this.location.from.Erasmus[data$Number.of.letters.sent.from.this.location.from.Erasmus < (quartiles[3] + 1.5 * IQR)])
...
plot <- ggplot(data, aes(x = " ", y = Number.of.letters.sent.from.this.location.from.Erasmus)) +
  ...
  geom_text_repel(label = ifelse(data$Number.of.letters.sent.from.this.location.from.Erasmus > upper_whisker, as.character(data$Location.Name), "")) +
  theme_bw() +
