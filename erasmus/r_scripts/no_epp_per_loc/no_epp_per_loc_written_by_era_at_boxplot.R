...
plot <- ggplot(data, aes(x= ' ', y = Number.of.letters.sent.from.this.location.from.Erasmus)) +
  ...
  geom_text(check_overlap = TRUE, aes(label=ifelse(Number.of.letters.sent.from.this.location.from.Erasmus>=68.5,as.character(Location.Name),'')),hjust=-0.1,vjust=0) +
  theme_bw() +
