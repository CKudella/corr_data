...
# calculate quartiles
quartiles <- as.numeric(quantile(data$Number.of.letters.sent.to.this.location.by.Erasmus, probs = c(0.25, 0.5, 0.75)))

# calculate IQR
IQR <- diff(quartiles[c(1, 3)])

# calculate outlier treshold
upper_dots <- min(data$Number.of.letters.sent.to.this.location.by.Erasmus[data$Number.of.letters.sent.to.this.location.by.Erasmus > (quartiles[3] + 1.5*IQR)])

# create box plot
plot <- ggplot(data, aes(x = " ", y = Number.of.letters.sent.to.this.location.by.Erasmus)) +
  geom_boxplot(outlier.size = 2, notch = FALSE) +
  geom_text_repel(box.padding = 2, max.overlaps = Inf, label = ifelse(data$Number.of.letters.sent.to.this.location.by.Erasmus >= upper_dots, as.character(data$Location.Name), "")) +
