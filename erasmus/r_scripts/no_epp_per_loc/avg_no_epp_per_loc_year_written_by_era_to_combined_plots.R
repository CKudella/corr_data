...
output_path <- "../r_plots/no_epp_per_loc/avg_no_epp_per_loc_year_written_by_era_to_combined_plots"
...
measure_label <- str_glue("average number of letters sent by {subject_name} per location per year")
...
plot_boxplot <- data |>
  ...
  labs(title = "Boxplot with labeled outliers", x = NULL,
       y = str_glue("Average number of letters sent by {subject_name} per year")) +
  theme_bw(base_size = 12) +
