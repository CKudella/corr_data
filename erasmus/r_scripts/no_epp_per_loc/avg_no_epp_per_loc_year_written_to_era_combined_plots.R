...
data_path <- "../query_results/no_epp_per_loc/avg_no_epp_per_loc_year_written_to_era.csv"
output_path <- "../r_plots/no_epp_per_loc/avg_no_epp_per_loc_year_written_to_era_combined_plots"

measure_col  <- str_glue("Average Number of Letters written from this location to {subject_name} per year")
location_col <- "Location Name"

measure_label <- str_glue("average number of letters written to {subject_name} per location per year")
...
plot_histogram <- data |>
  ...
  labs(
    ...
    x = str_glue("Average number of letters written to {subject_name} per year (log-scale)"),
...
plot_boxplot <- data |>
  ...
  labs(title = "Boxplot with labeled outliers", x = NULL,
       y = str_glue("Average number of letters written to {subject_name} per year")) +
  theme_bw(base_size = 12) +
