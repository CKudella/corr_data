...
plot <- ggplot(data=daten, aes(x= reorder(Location.Name.Modern, -Number.of.letters.sent.FROM.this.location), y=Number.of.letters.sent.FROM.this.location, label=Location.Name.Modern)) + geom_point(stat = "identity") + labs(x="Modern Location",y="Number of letters sent") + geom_text(check_overlap = TRUE, aes(label=ifelse(Number.of.letters.sent.FROM.this.location>80,as.character(Location.Name.Modern),'')),hjust=-0.35,vjust=0) 
