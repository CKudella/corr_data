...
upper_whisker <- max(data$Number.of.letters.sent.from.this.location[data$Number.of.letters.sent.from.this.location < (quartiles[3] + 1.5 * IQR)])

# create boxplot
plot <- ggplot(data, aes(x = " ", y = Number.of.letters.sent.from.this.location.to.Erasmus)) +
  geom_boxplot(outlier.size = 2, notch = FALSE) +
  geom_text_repel(box.padding = 1.25, label = ifelse(data$Number.of.letters.sent.from.this.location.to.Erasmus > upper_whisker, as.character(data$Location.Name), "")) +
  theme_bw() +
