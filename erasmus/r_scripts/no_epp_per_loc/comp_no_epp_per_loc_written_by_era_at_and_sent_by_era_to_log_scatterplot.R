...
data_path <- "../query_results/no_epp_per_loc/comp_epp_per_loc_written_by_era_at_and_sent_by_era_to.csv"
output_path <- "../r_plots/no_epp_per_loc/comp_no_epp_per_loc_written_by_era_at_and_sent_by_era_to_log_scatterplot"

x_col        <- "NoLettersWrittenByErasmusTO"
y_col        <- "NoLettersWrittenBYErasmusAT"
location_col <- "LocationName"

x_label <- str_glue("Number of letters sent BY {subject_name} TO this location")
y_label <- str_glue("Number of letters written BY {subject_name} AT this location")

shared_breaks <- c(1, 5, 25, 50, 100, 200, 350, 500, 675)
...
plot_scatter <- data |>
  ...
  coord_fixed(ratio = 1) +
  labs(
    title = str_glue("Locations as writing base vs. destination of {subject_name}'s letters"),
    subtitle = "dashed line = equilibrium \u00b7 square-root scale (identical for both axes)",
    x = x_label,
    y = y_label
  ) +
