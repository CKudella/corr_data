...
data_path <- "../query_results/no_epp_per_loc/avg_no_epp_per_loc_year_written_by_era_at.csv"
output_path <- "../r_plots/no_epp_per_loc/avg_no_epp_per_loc_year_written_by_era_at_combined_plots"

measure_col  <- str_glue("Average Number of Letters written by {subject_name} from this location per year")
location_col <- "Location Name"

measure_label <- str_glue("average number of letters written by {subject_name} per location per year")
...
plot_histogram <- data |>
  ...
  labs(
    title = str_glue("Distribution of the {measure_label}"),
    subtitle = str_glue("Mean = {round(mean_letters, 1)} \u00b7 Median = {median_letters}"),
    x = str_glue("Average number of letters written by {subject_name} per year (log-scale)"),
...
plot_boxplot <- data |>
  ...
  labs(title = "Boxplot with labeled outliers", x = NULL,
       y = str_glue("Average number of letters written by {subject_name} per year")) +
  theme_bw(base_size = 12) +
...
plot_barchart <- outliers |>
  ...
  labs(
    title = str_glue("All {n_outliers} outlier locations (> {round(upper_fence, 1)} letters per year on average)"),
    x = "Location", y = "Average number of letters per year"
  ) +
  theme_bw(base_size = 12) +
