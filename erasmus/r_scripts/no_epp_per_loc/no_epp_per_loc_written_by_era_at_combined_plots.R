...
data_path <- "../query_results/no_epp_per_loc/no_epp_per_loc_written_by_era_at.csv"
output_path <- "../r_plots/no_epp_per_loc/no_epp_per_loc_written_by_era_at_combined_plots"

measure_col  <- str_glue("Number of letters sent from this location by {subject_name}")
...
cat(str_glue("mean letters: {round(mean_letters, 1)}"), "\n")
cat(str_glue("median letters: {median_letters}"), "\n")
...
plot_histogram <- data |>
  ...
  labs(
    title = "Distribution of letters per location",
...
plot_boxplot <- data |>
  ...
  labs(title = "Boxplot with labeled outliers", x = NULL, y = "Number of letters") +
  theme_bw(base_size = 12) +
...
plot_barchart <- outliers |>
  ...
  labs(
    title = str_glue("All {n_outliers} outlier locations (> {round(upper_fence, 1)} letters)"),
    x = "Location", y = "Number of letters"
  ) +
  theme_bw(base_size = 12) +
