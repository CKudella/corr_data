...
# create bar chart
plot <- ggplot(data = data, aes(x = reorder(Location.Name, -Number.of.letters.sent.from.this.location.by.Erasmus), y = Number.of.letters.sent.from.this.location.by.Erasmus, label = Location.Name)) +
  geom_bar(stat = "identity") +
  labs(x = "Location", y = "Number of letters written by Erasmus at this location") +
  geom_text(aes(label = Number.of.letters.sent.from.this.location.by.Erasmus), vjust = -0.5, color = "black") +
  theme_bw() +
