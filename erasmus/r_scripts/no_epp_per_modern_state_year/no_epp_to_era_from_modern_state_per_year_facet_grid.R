...
data_path <- "../query_results/no_epp_per_modern_state_year/no_epp_to_era_from_modern_state_per_year.csv"
output_path <- "../r_plots/no_epp_per_modern_state_year/no_epp_to_era_from_modern_state_per_year_facet_grid"

measure_col <- str_glue("Number of letters sent from this modern state to {subject_name} this year")
...
plot_facets <- data |>
  ...
  labs(
    title = str_glue("Letters sent TO {subject_name} FROM each modern state, over time"),
    subtitle = "states ordered by total number of letters sent",
    x = "Year",
    y = "Number of letters"
  ) +
