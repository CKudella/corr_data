...
daten<-read.csv("data/comp_no_epp_written_by_era_to_no_corr_per_year.csv")
...
plot + theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.35)) + theme(legend.position="bottom") + scale_color_grey(labels = c("Number of letters written by Erasmus", "Number of correspondents receiving letters from Erasmus"))