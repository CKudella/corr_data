...
data_path <- "../query_results/no_epp_per_modern_state/no_epp_per_modern_state_sent_to_era.csv"
output_path <- "../r_plots/no_epp_per_modern_state/no_epp_per_modern_state_sent_to_era_barchart"

measure_col <- str_glue("Number of letters sent from this modern state to {subject_name}")
...
plot_barchart <- data |>
  ...
  labs(
    title = str_glue("Number of letters sent TO {subject_name} FROM locations in each modern state"),
