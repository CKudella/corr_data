...
# settings
# note: title/scope of this dataset (all correspondence vs. subject-specific)
# was left unresolved in an earlier discussion - verify before relying on it

subject_name <- "Erasmus"

data_path <- "../query_results/no_epp_per_modern_state/no_epp_sent_from_modern_state.csv"
output_path <- "../r_plots/no_epp_per_modern_state/no_epp_sent_from_modern_state_barchart"

measure_col <- "Number of letters sent from this modern state"
...
plot_barchart <- data |>
  ...
  labs(
    title = "Number of letters sent FROM locations in each modern state",
