...
upper_whisker <- max(data$Average.number.of.letters.sent.per.correspondent.to.Erasmus.this.year[data$Average.number.of.letters.sent.per.correspondent.to.Erasmus.this.year < (quartiles[3] + 1.5 * IQR)])
...
plot2 <- ggplot(data, aes(x = " ", y = Average.number.of.letters.sent.per.correspondent.to.Erasmus.this.year)) +
  ...
  geom_text_repel(label = ifelse(data$Average.number.of.letters.sent.per.correspondent.to.Erasmus.this.year > upper_whisker, as.character(data$Year), "")) +
  theme_bw() +
