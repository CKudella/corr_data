...
# create bar chart
plot <- ggplot(data, aes(x = Year, y = Average.number.of.letters.sent.by.Erasmus.per.correspondent.this.year)) +
  geom_bar(position = "dodge", stat = "identity") +
  geom_text(aes(label =  sprintf("%.2f", Average.number.of.letters.sent.by.Erasmus.per.correspondent.this.year)), vjust = -0.5, color = "black", size = 2.5) +
  geom_hline(aes(yintercept = mean(Average.number.of.letters.sent.by.Erasmus.per.correspondent.this.year), linetype = "mean"), size = 0.3) +
  geom_hline(aes(yintercept = median(Average.number.of.letters.sent.by.Erasmus.per.correspondent.this.year), linetype = "median"), size = 0.3) +
