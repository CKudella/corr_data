...
data_path <- "../query_results/no_corr_per_year/avg_no_epp_per_corr_year_from_era.csv"
output_path <- "../r_plots/no_corr_per_year/avg_no_epp_per_corr_year_from_era_combined_plots"

measure_col <- str_glue("Average number of letters sent by {subject_name} per correspondent this year")
year_col    <- "Year"
...
plot_histogram <- data |>
  ...
  labs(
    title = str_glue("Distribution of the average number of letters sent by {subject_name} per correspondent, by year"),
...
plot_boxplot <- data |>
  ...
  labs(
    ...
    x = NULL,
    y = str_glue("Average number of letters sent by {subject_name} per correspondent")
  ) +
  theme_bw(base_size = 12) +
