...
data_path <- "../query_results/no_corr_per_year/comp_no_corr_no_epp_per_year_to_era.csv"
output_path <- "../r_plots/no_corr_per_year/comp_no_corr_no_epp_per_year_to_era_combined_plots"

correspondents_col <- str_glue("Number of correspondents writing letters to {subject_name} this year")
letters_col        <- str_glue("Number of letters sent to {subject_name} this year")
year_col           <- "Year"
...
plot_correspondents <- data |>
  ...
  labs(
    title = str_glue("Number of correspondents who wrote letters to {subject_name}, by year"),
...
plot_letters <- data |>
  ...
  scale_y_continuous(labels = comma, breaks = pretty_breaks(n = 8)) +
  labs(
    title = str_glue("Number of letters sent to {subject_name}, by year"),
