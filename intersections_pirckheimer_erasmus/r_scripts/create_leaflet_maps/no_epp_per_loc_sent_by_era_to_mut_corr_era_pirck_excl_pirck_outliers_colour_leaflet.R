...
# Determine breaks
median_val <- median(data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer, na.rm = TRUE)
q1 <- quantile(data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer)
...
breaks <- c(
  min(data$Number.of.letters.sent.to.this.location.by.Erasmus, na.rm = TRUE),
  median_val, q3, outliers, max(data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer, na.rm = TRUE)
)
...
data$class <- findInterval(data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name.Modern, "</b><br>Number of letters sent by Erasmus </b> to mutual correspondents of his and </b> Pirckheimer (excl. Pirckheimer): ", data$Number.of.letters.sent.to.this.location.by.Erasmus.to.mutual.correspondents.of.his.and.Pirckheimer..excl..Pirckheimer),
    group = "Locations",
...
# Round all breaks to the bigger whole number
breaks_numeric <- ceiling(breaks_numeric)
    ...
    sapply(1:(length(breaks_numeric) - 1), function(i) {
      ...
      if (lower_bound == upper_bound) {
        if (lower_bound == 1) {
          ...
          label_text <- paste0(format_numbers(lower_bound), " letters")
        }
      } else {
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters sent by Erasmus to mutual correspondents of his and Pirckheimer (excl. Pirckheimer)
</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_sent_by_era_to_mut_corr_era_pirck_excl_pirck_outliers_colour.html", selfcontained = TRUE)
