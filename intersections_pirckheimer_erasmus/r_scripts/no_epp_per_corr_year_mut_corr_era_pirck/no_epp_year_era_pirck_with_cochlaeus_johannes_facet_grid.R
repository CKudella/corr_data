...
# read data
data<-read.csv("no_epp_per_corr_year_mut_corr_era_pirck/no_epp_year_era_pirck_with_cochlaeus_johannes.csv", fileEncoding="UTF-8", na.strings=c("NULL"))
...
data_long <- data %>%
  ...
  mutate(
    ...
    direction = case_when(
      variable %in% c("EPPEtX", "EPPPtX") ~ "sent to Cochlaeus",
      variable %in% c("EPPXtE", "EPPXtP") ~ "received from Cochlaeus"
    )
  )
...
plot <- ggplot(data_complete, aes(x = Year, y = signed_value, fill = direction)) +
  ...
  labs(
    ...
    subtitle = "Grey bands mark years in which Erasmus and Pirckheimer were both corresponding with Cochlaeus"
  ) +
...
ggsave("no_epp_year_era_pirck_with_cochlaeus_johannes_facet_grid.pdf", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
ggsave("no_epp_year_era_pirck_with_cochlaeus_johannes_facet_grid.png", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
ggsave("no_epp_year_era_pirck_with_cochlaeus_johannes_facet_grid.eps", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
ggsave("no_epp_year_era_pirck_with_cochlaeus_johannes_facet_grid.svg", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
