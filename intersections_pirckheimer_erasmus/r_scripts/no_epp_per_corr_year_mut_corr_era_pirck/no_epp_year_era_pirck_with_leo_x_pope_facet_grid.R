...
data_long <- data %>%
  ...
  mutate(
    ...
    direction = case_when(
      variable %in% c("EPPEtX", "EPPPtX") ~ "sent to Leo X",
      variable %in% c("EPPXtE", "EPPXtP") ~ "received from Leo X"
    )
  )
...
data_complete <- data_long %>%
  ...
  mutate(signed_value = if_else(direction == "sent to Leo X", value, -value))
...
plot <- ggplot(data_complete, aes(x = Year, y = signed_value, fill = direction)) +
  ...
  scale_fill_manual(values = c("sent to Leo X" = "grey20",
                               "received from Leo X" = "grey65"),
                    name = NULL) +
  ...
  labs(
    ...
    title = "Correspondence with Pope Leo X",
    subtitle = "Grey bands mark years in which Erasmus and Pirckheimer were both corresponding with Leo X"
  ) +
