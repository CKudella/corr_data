...
# read data
data<-read.csv("no_epp_per_corr_year_mut_corr_era_pirck/no_epp_year_era_pirck_with_lange_johannes.csv", fileEncoding="UTF-8", na.strings=c("NULL"))
...
data_long <- data %>%
  ...
  mutate(
    ...
    direction = case_when(
      variable %in% c("EPPEtX", "EPPPtX") ~ "sent to Lang",
      variable %in% c("EPPXtE", "EPPXtP") ~ "received from Lang"
    )
  )
...
data_complete <- data_long %>%
  ...
  mutate(signed_value = if_else(direction == "sent to Lang", value, -value))
...
plot <- ggplot(data_complete, aes(x = Year, y = signed_value, fill = direction)) +
  ...
  scale_fill_manual(values = c("sent to Lang" = "grey20",
                               "received from Lang" = "grey65"),
                    name = NULL) +
  ...
  labs(
    ...
    title = "Correspondence with Johann Lang",
    subtitle = "Grey bands mark years in which Erasmus and Pirckheimer were both corresponding with Lang"
  ) +
