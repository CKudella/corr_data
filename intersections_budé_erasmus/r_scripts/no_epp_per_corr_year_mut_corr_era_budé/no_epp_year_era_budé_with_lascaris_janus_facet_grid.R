...
# read data
data<-read.csv("no_epp_per_corr_year_mut_corr_era_budé/no_epp_year_era_budé_with_lascaris_janus.csv", fileEncoding="UTF-8", na.strings=c("NULL"))
...
data_long <- data %>%
  ...
  mutate(
    ...
    direction = case_when(
      variable %in% c("EPPEtX", "EPPBtX") ~ "sent to Lascaris",
      variable %in% c("EPPXtE", "EPPXtB") ~ "received from Lascaris"
    )
  )
...
data_complete <- data_long %>%
  ...
  mutate(signed_value = if_else(direction == "sent to Lascaris", value, -value))
...
plot <- ggplot(data_complete, aes(x = Year, y = signed_value, fill = direction)) +
  ...
  scale_fill_manual(values = c("sent to Lascaris" = "grey20",
                               "received from Lascaris" = "grey65"),
                    name = NULL) +
  ...
  labs(
    x = "Year",
    y = "Number of letters",
    title = "Correspondence with Janus Lascaris",
    subtitle = "Grey bands mark years in which Erasmus and Budé were both corresponding with Lascaris"
  ) +
...
ggsave("no_epp_year_era_budé_with_lascaris_janus_facet_grid.pdf", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
ggsave("no_epp_year_era_budé_with_lascaris_janus_facet_grid.png", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
ggsave("no_epp_year_era_budé_with_lascaris_janus_facet_grid.eps", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
ggsave("no_epp_year_era_budé_with_lascaris_janus_facet_grid.svg", plot = last_plot(), scale = 1, width = 11.7, height = 4.15, units = "in", dpi = 600, limitsize = TRUE)
