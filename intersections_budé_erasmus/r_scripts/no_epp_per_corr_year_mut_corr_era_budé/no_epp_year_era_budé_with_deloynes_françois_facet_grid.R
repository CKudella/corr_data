...
data_long <- data %>%
  ...
  mutate(
    ...
    direction = case_when(
      variable %in% c("EPPEtX", "EPPBtX") ~ "sent to Deloynes",
      variable %in% c("EPPXtE", "EPPXtB") ~ "received from Deloynes"
    )
  )
...
data_complete <- data_long %>%
  ...
  mutate(signed_value = if_else(direction == "sent to Deloynes", value, -value))
...
plot <- ggplot(data_complete, aes(x = Year, y = signed_value, fill = direction)) +
  ...
  scale_fill_manual(values = c("sent to Deloynes" = "grey20",
                               "received from Deloynes" = "grey65"),
                    name = NULL) +
  ...
  labs(
    x = "Year",
    y = "Number of letters",
    title = "Correspondence with François Deloynes",
    subtitle = "Grey bands mark years in which Erasmus and Budé were both corresponding with Deloynes"
  ) +
