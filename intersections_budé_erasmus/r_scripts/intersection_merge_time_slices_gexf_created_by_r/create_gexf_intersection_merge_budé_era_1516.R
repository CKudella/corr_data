...
nodes <- read.csv("time_slice_1516_intersection_merge_budé_and_era_correspondents.csv", fileEncoding="UTF-8")
links <- read.csv("time_slice_1516_intersection_merge_budé_and_era_letters_corr_as_nodes.csv", fileEncoding="UTF-8")[ ,c('Source', 'Target')]
...
# write gexf
era_budé_imerge_1516 <- write.gexf(nodes = nodes_df, edges = edges_df, edgesWeight = E(net2)$weight, nodesAtt = nodes_att, nodesVizAtt = nodes_att_viz, edgesVizAtt = edges_att_viz, defaultedgetype = "directed",  meta = list( creator="Christoph Kudella",  description="A graph representing the intersection between Erasmus's and Budé's networks of correspondence in the year 1516 (only showing epistolary contacts with mutal correspondents)"), output="era_budé_imerge_1516.gexf")