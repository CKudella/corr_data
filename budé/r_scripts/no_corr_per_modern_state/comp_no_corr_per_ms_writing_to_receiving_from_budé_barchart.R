...
data_long <- data %>% pivot_longer(cols = c("Number.of.correspondents.who.received.letters.from.Budé", "Number.of.correspondents.who.wrote.letters.to.Budé"), names_to = "variable", values_to = "value")
...
plot <- ggplot(data_long, aes(x = reorder(Modern.State, -value), y = value, fill = variable)) +
  ...
  scale_fill_grey(labels = c("Number of correspondents to whom Budé wrote letters", "Number of correspondents who wrote letters to Budé"))
plot
