...
plot <- ggplot(duration_of_correspondence_reciproc_corr, aes(x = " ", y = duration_in_years)) +
  ...
  geom_text_repel(label = ifelse(duration_of_correspondence_reciproc_corr$duration_in_years >= upper_dots, as.character(duration_of_correspondence_reciproc_corr$name_in_edition), ""), box.padding = 0.5, max.overlaps = Inf) +
