...
plot1 <- ggplot(data3, aes(x = Year, y = Average.number.of.letters.sent.by.Budé.per.correspondent.this.year)) +
...
# create box plot
plot2 <- ggplot(data3, aes(x = " ", y = Average.number.of.letters.sent.by.Budé.per.correspondent.this.year)) +
  geom_boxplot(notch = FALSE) +
  geom_text_repel(label = ifelse(data3$Average.number.of.letters.sent.by.Budé.per.correspondent.this.year >= upper_dots, as.character(data3$Year), "")) +
  labs(x = "Year", y = "Average number of letters sent by Budé per year and correspondent") +
  theme_bw() +
