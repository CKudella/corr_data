...
plot <- ggplot(data, aes(x = " ", y = Average.number.of.letters.per.correspondent)) +
  ...
  geom_text_repel(label = ifelse(data$Average.number.of.letters.per.correspondent >= upper_dots, as.character(data$Location.Name), "")) +
  labs(x = "Location", y = "Average number of letters sent by Budé to this location per correspondent")
  theme_bw() +
