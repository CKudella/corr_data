...
num_breaks <- 3
breaks <- classIntervals(data$Number.of.letters.sent.from.this.location.to.Budé, n = num_breaks, style = "jenks")$brks
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Number of letters sent to <br/> Budé from this location: ", data$Number.of.letters.sent.from.this.location.to.Budé),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
saveWidget(m, file = "no_epp_per_loc_sent_to_budé_with_geocoordinates_colour.html", selfcontained = TRUE)
