...
# Determine breaks
median_val <- median(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, na.rm = TRUE)
q1 <- quantile(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé)
upper_whisker <- box_stats$stats[5]
outliers <- min(box_stats$out)

# Calculate natural breaks
num_breaks <- 3
breaks <- classIntervals(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, n = num_breaks, style = "jenks")$brks
breaks[1] <- min(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, na.rm = TRUE)
breaks[4] <- max(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, na.rm = TRUE)
...
data$class <- findInterval(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name.Modern, "</b><br>Number of correspondents who wrote <br/> from this location to Budé: ", data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé),
    group = "Locations",
  ...
  paste0(
    sapply(1:(length(breaks_numeric) - 1), function(i) {
      ...
      if (i == 1 && lower_bound == upper_bound) {
        label_text <- paste0(format_numbers(lower_bound), " correspondent")
      } else {
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of correspondents who wrote from this location to Budé</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_corr_per_loc_writing_to_budé_with_geocoordinates_colour.html", selfcontained = TRUE)
