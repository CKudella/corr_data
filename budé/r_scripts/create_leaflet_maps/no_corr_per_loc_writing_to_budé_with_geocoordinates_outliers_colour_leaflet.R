...
breaks <- c(min(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, na.rm = TRUE), 
            q3, outliers, max(data$Number.of.correspondents.who.wrote.from.this.location.letters.to.Budé, na.rm = TRUE))
    ...
    sapply(1:(length(breaks_numeric) - 1), function(i) {
      ...
      if (lower_bound == upper_bound) {
        if (lower_bound == 1) {
          ...
          label_text <- paste0(format_numbers(lower_bound), " correspondents")
        }
      } else {
...
saveWidget(m, file = "no_corr_per_loc_writing_to_budé_with_geocoordinates_outliers_colour.html", selfcontained = TRUE)
