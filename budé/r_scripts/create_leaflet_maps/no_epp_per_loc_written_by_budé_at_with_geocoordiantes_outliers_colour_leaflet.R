...
# Read data
data <- read.csv("no_epp_per_loc/no_epp_per_loc_written_by_budé_at_with_geocoordiantes.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Determine breaks
median_val <- median(data$Number.of.letters.sent.from.this.location.from.Budé, na.rm = TRUE)
q1 <- quantile(data$Number.of.letters.sent.from.this.location.from.Budé, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.letters.sent.from.this.location.from.Budé, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.letters.sent.from.this.location.from.Budé)
...
breaks <- c(min(data$Number.of.letters.sent.from.this.location.from.Budé, na.rm = TRUE), 
            q3, outliers, max(data$Number.of.letters.sent.from.this.location.from.Budé, na.rm = TRUE))
...
data$class <- findInterval(data$Number.of.letters.sent.from.this.location.from.Budé, breaks_numeric, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location.Name, "</b><br>Number of letters written by <br> Budé at this location: ", data$Number.of.letters.sent.from.this.location.from.Budé),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
legend_html <- paste0(
  ...
  paste0(
    sapply(1:(length(breaks_numeric) - 1), function(i) {
      ...
      upper_bound <- if (i == length(breaks_numeric) - 1) breaks_numeric[i + 1] else breaks_numeric[i + 1] - 1
      
      paste0(
        "<div style='display: flex; align-items: center; margin-bottom: 5px;'>",
        "<div style='width: ", (i * 5), "px; height: ", (i * 5), 
        "px; background-color: #C3161F; border-radius: 50%; margin-right: 10px;'></div>",
        "<span>", lower_bound, if (lower_bound == upper_bound) "" else paste0(" - ", upper_bound), " letters</span>",
        "</div>"
      )
    }),
    collapse = ""
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters written by Budé at this location</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_written_by_budé_at_with_geocoordiantes_outliers_colour.html", selfcontained = TRUE)
