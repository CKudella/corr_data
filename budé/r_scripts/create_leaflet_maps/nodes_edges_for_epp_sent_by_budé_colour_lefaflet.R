...
breaks <- c(min(edges_with_weights$weight, na.rm = TRUE), 
...
m <- m %>% 
  addPulseMarkers(
    ...
    popup = ~paste0("<b>", Label, "</b><br>Number of letters sent by Budé within this city: ", weight),  # Use weight
...
m <- m %>%
  addCircleMarkers(
    ...
    lng = ~locations_lng,
    lat = ~locations_lat,
    popup = paste("<b>", nodes$Label, "</b><br>Number of letters sent by Budé to this location : ", nodes$degree),
    group = "Nodes",
    label = ~Label,
    radius = 2,
    fillColor = "#C3161F",
...
saveWidget(m, file = "nodes_edges_for_epp_sent_by_budé_colour.html", selfcontained = TRUE)
m