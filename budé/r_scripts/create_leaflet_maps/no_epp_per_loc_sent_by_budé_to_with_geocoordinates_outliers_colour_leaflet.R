...
# Read data
data <- read.csv("no_epp_per_loc/no_epp_per_loc_sent_by_budé_to_with_geocoordinates.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Determine breaks
median_val <- median(data$Number.of.letters.sent.to.this.location.from.Budé, na.rm = TRUE)
q1 <- quantile(data$Number.of.letters.sent.to.this.location.from.Budé, 0.25, na.rm = TRUE)
q3 <- quantile(data$Number.of.letters.sent.to.this.location.from.Budé, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Number.of.letters.sent.to.this.location.from.Budé)
...
breaks <- c(min(data$Number.of.letters.sent.to.this.location.from.Budé, na.rm = TRUE), 
            q3, outliers, max(data$Number.of.letters.sent.to.this.location.from.Budé, na.rm = TRUE))
...
data$class <- findInterval(data$Number.of.letters.sent.to.this.location.from.Budé, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>",data$Location, "</b><br>Number of letters sent by <br/> Budé to this location: ", data$Number.of.letters.sent.to.this.location.from.Budé),
    group = "Locations",
    label = ~Location,
    radius = data$class * 3,
    fillColor  = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Number of letters sent by Budé to this location</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "no_epp_per_loc_sent_by_budé_to_with_geocoordinates_colour.html", selfcontained = TRUE)
