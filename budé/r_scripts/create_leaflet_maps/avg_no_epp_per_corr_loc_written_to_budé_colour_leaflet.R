...
# Read data
data <- read.csv("no_epp_per_loc/avg_no_epp_per_corr_loc_written_to_budé.csv", fileEncoding = "UTF-8", na.strings = c("NULL"))

# Determine breaks
median_val <- median(data$Average.number.of.letters, na.rm = TRUE)
q1 <- quantile(data$Average.number.of.letters, 0.25, na.rm = TRUE)
q3 <- quantile(data$Average.number.of.letters, 0.75, na.rm = TRUE)
box_stats <- boxplot.stats(data$Average.number.of.letters)
upper_whisker <- box_stats$stats[5]
outliers <- min(box_stats$out)

# Calculate natural breaks
num_breaks <- 3
breaks <- classIntervals(data$Average.number.of.letters, n = num_breaks, style = "jenks")$brks
...
data$class <- findInterval(data$Average.number.of.letters, breaks, rightmost.closed = TRUE)
...
m <- leaflet(data) %>%
  ...
  addCircleMarkers(
    lng = ~Longitude,
    lat = ~Latitude,
    popup = paste("<b>", data$Location.Name, "</b><br>Average number of letters sent to Budé <br> from this location per correspondent ", data$Average.number.of.letters),
    group = "Locations",
    label = ~Location.Name,
    radius = data$class * 3,
    fillColor = "#C3161F",
...
m <- m %>%
  addControl(
    html = "<div style='width: 100%; text-align: center;'>
              ...
              Average number of letters sent to Budé from this location per correspondent</h3>
            </div>",
    position = "topright"
  )
...
saveWidget(m, file = "avg_no_epp_per_corr_loc_written_to_budé_colour.html", selfcontained = TRUE)
