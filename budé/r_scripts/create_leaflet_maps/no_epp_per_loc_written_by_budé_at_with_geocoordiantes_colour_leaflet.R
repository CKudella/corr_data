...
num_breaks <- 4
breaks <- classIntervals(data$Number.of.letters.sent.from.this.location.from.Budé, n = num_breaks, style = "jenks")$brks
...
data$class <- findInterval(data$Number.of.letters.sent.from.this.location.from.Budé, breaks, rightmost.closed = TRUE)
...
saveWidget(m, file = "no_epp_per_loc_written_by_budé_at_with_geocoordiantes_colour.html", selfcontained = TRUE)
